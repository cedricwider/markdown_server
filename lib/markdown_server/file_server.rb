...
class MarkdownServer < Sinatra::Application
  ...
  get '/:filename' do
    ...
    haml :read, locals: { text: html_content_from(markup: filename), title: title, files: available_files }
  end
  
  get '/edit/:filename' do
    haml :edit, locals: { text: markup_content_from(file: filename), title: title }
  end
